...
import { Home, Map as MapIcon, Sword, BookOpen } from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = ({ activeScreen, onScreenChange }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-6 pb-8 pt-4 bg-[#131313]/90 backdrop-blur-2xl rounded-t-[3rem] shadow-[0_-8px_32px_rgba(229,226,225,0.05)]">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={cn(
              "flex flex-col items-center justify-center transition-all duration-300 px-4 py-2",
              isActive 
          ...
          >
            <item.icon size={24} className={cn("mb-1", isActive && "fill-current")} />
